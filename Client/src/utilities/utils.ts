...
export function applyQuaternion(vec: THREE.Vector3, quat: THREE.Quaternion) {
  const x = vec.x,
    y = vec.y,
    z = vec.z;
  const qx = quat.x,
...
export function getYawRotation(rotation: THREE.Quaternion) {
  ...
  const yawRot = new THREE.Quaternion().setFromAxisAngle(
...
export function getPitchRotation(rotation: THREE.Quaternion) {
  const incomingQuaternion = new THREE.Quaternion(
    rotation.x,
    rotation.y,
    rotation.z,
  ...
  const pitchRot = new THREE.Quaternion().setFromAxisAngle(
...
export function getPitchandYawRotation(rotation: THREE.Quaternion) {
  const incomingQuaternion = new THREE.Quaternion(
    rotation.x,
    rotation.y,
    rotation.z,

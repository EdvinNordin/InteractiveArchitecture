...
export function setObjectCells(object: THREE.Object3D, grid: any) {
    ...
    for (let x = cellStart(objectMinX); x < cellEnd(objectMaxX); x = x + cellSize) {
        for (let y = cellStart(objectMinY); y < cellEnd(objectMaxY); y = y + cellSize) {
            for (let z = cellStart(objectMinZ); z < cellEnd(objectMaxZ); z = z + cellSize) {
...
export function getObjectsInCell(position: THREE.Vector3, grid: { [key: string]: THREE.Object3D[] }) {
